...
class Player extends Human implements CommandSender, ChunkLoader, IPlayer, NetworkSession
{
	...
	public function isValidUserName(string $name): bool
	{
		...
		$len = strlen($name);
		if($this->server->getLeverylConfigValue("AllowUserNamesWithSpaces", false)) {
				...
				if($this->speed !== null) {
					if($this->onGround) {
						...
						if(!$this->allowFlight and $this->inAirTicks > 10 and !$this->isSleeping() and !$this->isImmobile()) {
							...
							if(!$this->hasEffect(Effect::JUMP) and $diff > 0.6 and $expectedVelocity < $this->speed->y and !$this->server->getAllowFlight() and !($this->getInventory()->getChestplate() instanceof Elytra)) {
	...
	public function save($async = false)
	{
		...
		$this->namedtag["Health"] = new ShortTag("Health", intval($this->getHealth()));
		$this->namedtag["MaxHealth"] = new ShortTag("MaxHealth", intval($this->getMaxHealth()));
