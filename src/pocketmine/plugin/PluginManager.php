				...
				foreach(new \RegexIterator(new \DirectoryIterator($directory), $loader->getPluginFilters()) as $file){
					...
					try{
						...
						if($description instanceof PluginDescription){
							...
							if(stripos(strtolower($name), "pocketmine") !== false or stripos(strtolower($name), "minecraft") !== false or stripos($name, "mojang") !== false){
                            ...
                            if(!$this->server->getLeverylConfigValue("DisablePluginAPIVersionCheck", true)) {
                                $compatible = false;
